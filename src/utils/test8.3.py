...
# Cuando seleccionaa una fila de la tablas automaticamente cambia el combobox del padre
...
class InfoWindow(QDialog):
    ...
    def __init__(self, data, parent=None):
        ...
        for table_name, table_data in data.items():
            ...
            header_labels = list(table_data.keys())
    ...
    def on_button_clicked(self):
        ...
        if self.info_window is None:
            self.info_window = InfoWindow(
                {self.comboBox.currentText(): data}, parent=self)
            self.info_window.row_selected.connect(
            ...
            if table_name not in self.info_window.tables:
                ...
                self.info_window.tables[table_name].setColumnCount(len(data))
                header_labels = list(data.keys())
                self.info_window.tables[table_name].setHorizontalHeaderLabels(
                    header_labels)
                self.info_window.layout().insertWidget(self.info_window.layout().count() -
                                                       1, self.info_window.tables[table_name])

            self.info_window.tables[table_name].setRowCount(
                self.info_window.tables[table_name].rowCount() + 1)
            for col, (field_id, value) in enumerate(data.items()):
                self.info_window.tables[table_name].setItem(
                    self.info_window.tables[table_name].rowCount() - 1, col, QTableWidgetItem(str(value)))
